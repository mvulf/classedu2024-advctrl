...
def test_cartpole_system():
    system: CartPoleWithMotor = callback.detach(CartPoleWithMotor)()
    print(
        system.compute_state_dynamics(
            None, np.array([[1, 2, 3, 4, 5]]), np.array([[6]])
        )
    )
    save_and_assert(
        np.allclose(
            ...
            np.array([[3.0, 4.0, 9.51362395, 3.53952662, 50.0]]),
        ),
